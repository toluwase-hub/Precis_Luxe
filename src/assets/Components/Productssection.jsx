...
const Productssection = ({ cart, setCart, showCart, setShowCart }) => {
  ...
  const previewwhatsappurl = previewProduct
    ? `https://wa.me/${PhoneNumber}?text=${encodeURIComponent(previewWhatsappOrder())}`
    : "null";
    ...
    <div id="Collection" className="scroll-mt-24">
      ...
      <div className="contain">
        <div className="flex items-center justify-between ">
          <h1 className="font-bold text-lg my-3 flex">
            PRODUCTS{" "}
            <span className="text-sm font-normal  h-5 w-15 bg-red-500 flex items-center justify-center text-white rounded-full">
              {" "}
          ...
          <div>
            {" "}
            <button className="flex items-center gap-2" onClick={handleviewAll}>
        ...
        <div className="flex  gap-3 items-center justify-center py-10  ">
          {categories.map((category) => (
            <button
              className={` px-4 py-2 rounded-2xl transition-all duration-200  ${activecategory === category ? "bg-black text-white" : "bg-gray-400"}`}
              onClick={() => HnadleFiltered(category)}
            >
              {category}
            </button>
          ))}
        ...
        <div className=" grid lg:grid-cols-5 grid-cols-2 md:grid-cols-3 gap-3 ">
          {product.slice(0, numberProduct).map((product) => (
            <div className="border border-black/10 px-2 py-2 relative ">
              <div className="absolute z-20 bg-black text-white w-10 flex items-center justify-center">
                {product.id}
              </div>
              <div
                className={`absolute animate-pulse right-0  px-1 z-20 ${product.Status === "In_stock" ? "bg-green-500 text-green-50" : "bg-red-500 text-red-50"}`}
              >
                {product.Status}
              </div>
              <img
                src={product.img}
                alt={product.id}
                className="w-full h-70 object-cover hover:rotate-y-180 transition-all duration-400"
              />
              <h1 className="line-clamp-1">{product.productName}</h1>
              <h1
                className={`${product.price >= 10000 ? "text-red-500" : "text-green-500"}`}
              >
                {product.price !== "USD" ? "#" : "$"}
                {product.price.toFixed(2)}{" "}
                <span className="font-light text-sm text-gray-500 italic line-through">
              ...
              <button
                className={` ${product.Status === "Out_of_stock" ?  "disabled:opacity-40 disabled:cursor-not-allowed bg-red-400" : ""} py-2 px-3 border hover:bg-black/90 bg-black text-white`}
              ...
              <button
                className="flex justify-center items-center gap-1 font-light text-sm pt-2"
                onClick={() => handlePreviewProdcut(product)}
              >
                Preview <FaLongArrowAltRight />
              </button>
            </div>
          ))}
        </div>
        ...
        <div
          className={` fixed border top-17 right-4 lg:right-10 md:right-10 z-40 flex flex-col gap-1 px-2 py-4 bg-white/50 backdrop-blur-lg  h-[89%] overflow-auto ${showCart ? "flex" : "hidden"} `}
        >
                ...
                <div className="flex gap-5 border px-2 py-2  ">
                  ...
                  <div>
                    ...
                    <button
                      ...
                      className="bg-red-700 text-white py-2 px-2"
                    >
                      Delete ❌
                    </button>
                  </div>
          ...
          <div className="mt-5  pb-2">
            <a
              className={`${cart.length <= 0 ? "hidden" : "block"}`}
              href={whatsappurl}
              target="_blank"
              rel="noreferrer"
            >
              <button
                className={`bg-black text-white px-2 py-2 hover:bg-black/90 `}
              >
                Order Now
              </button>
            </a>
                ...
                {previousCart.map((previousCart) => (
                  <div className="flex gap-5  border px-2 py-2   ">
                    ...
                    <div>
                      ...
                      <button
                        onClick={() => addBackitem(previousCart)}
                        className="bg-red-700 text-white py-2 px-2"
                      >
                        Add
                      </button>
        ...
        {/* preview */}
        <div
          className={`fixed top-0 z-50 right-0 overflow-auto h-screen bg-white transition-all duration-300 ${showPreview ? "translate-x-0" : "translate-x-full"}`}
        >
          {previewProduct && (
            <div className="px-4 py-5">
              ...
              <div
                className={` ${previewProduct.Status === "Out_of_stock" ? "px-2  py-3" : "px-2" }  ${previewProduct.Status === "In_stock" ? "bg-green-500 text-green-50" : "bg-red-500 text-red-50"}`}
              >
                {previewProduct.Status}
              </div>

              <div className={`flex gap-1 ${ previewProduct.Status === "Out_of_stock" ? "hidden" : "flex"}`}>
                <Button
        ...
        <div className="flex  items-center justify-between my-5">
          <button
            className={`flex items-center ${numberProduct <= 10 ? "hidden" : "flex"} gap-2`}
            onClick={handleReduceProduct}
          >
            View less <FaLongArrowAltRight />{" "}
          </button>
          <button
            className={`flex items-center gap-2 ${numberProduct >= 40 ? "hidden" : "flex"}`}
